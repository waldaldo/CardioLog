...
export function AreaChart({ readings, width = 340, height = 200, showZones = true }: Props) {
  ...
  return (
    <Svg width={width} height={height}>
      ...
      <Path d={sysPath} fill="none" stroke="#00f0ff" strokeWidth={2}/>
      <Path d={diaPath} fill="none" stroke="#a78bfa" strokeWidth={2}/>
      <Line x1={pad.left} x2={width - pad.right} y1={yOf(130)} y2={yOf(130)}
            stroke="#facc15" strokeWidth={1} strokeDasharray="3 3" opacity={0.6}/>
      <Circle cx={xOf(readings.length - 1)} cy={yOf(readings[readings.length - 1].sys)}
              r={4} fill="#00f0ff" stroke="#07070a" strokeWidth={2}/>
    </Svg>
  );
}
